    ...
    with col2:
        if st.button(f"🎤", key=f"voice_{key}", help="Click to speak"):
            with st.spinner("Listening..."):
                lang_code = 'te-IN' if st.session_state.get('language') == 'తెలుగు' else 'hi-IN' if st.session_state.get('language') == 'हिन्दీ' else 'en-US'
...
<style>
    .main-header {
        background: linear-gradient(135deg, #4caf50 0%, #a8e6cf 100%);
        padding: 2rem;
        border-radius: 10px;
        text-align: center;
        color: white;
...
<div class="main-header">
    <h1>🌿 Welcome to AI Agri Voice Assistant</h1>
    <p>Get weather forecasts and crop recommendations using voice commands or manual input.</p>
</div>
""", unsafe_allow_html=True)

# Language selection
st.markdown("### 🌐 Choose Your Language")
lang = st.radio("", ["English", "తెలుగు", "हिन्दీ"], horizontal=True)
st.session_state['language'] = lang

# Input section for weather or crop prediction
st.markdown("### 📥 Enter your command:")
command = st.text_input("Type your command (e.g., 'weather' or 'crop prediction')", key="command_input")

if st.button("🔍 Submit Command"):
    if command.lower() == "weather":
        st.session_state['mode'] = 'weather'
        st.session_state['city_input'] = ""
    elif command.lower() == "crop prediction":
        st.session_state['mode'] = 'crop'
    else:
        st.warning("Please enter a valid command: 'weather' or 'crop prediction'.")

# Weather Section
if st.session_state.get('mode') == 'weather':
    st.markdown("""
...
if st.session_state.get('mode') == 'crop':
    st.markdown("""
    <div class="section-card">
